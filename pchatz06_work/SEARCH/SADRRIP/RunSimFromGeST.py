...
root_dir = sys.argv[2]
after_dir = sys.argv[3]
bench_list = sys.argv[4]


error = open(f"../BENCH_DIR/{root_dir}/{after_dir}/error.out", 'a')


# Get a list of all files in the directory
file_list = os.listdir(f'../BENCH_DIR/{root_dir}/{after_dir}/Individuals')
...
# Create a bash file
os.makedirs(f"../BENCH_DIR/{root_dir}/{after_dir}", exist_ok=True)
sbatch = open(f"../BENCH_DIR/{root_dir}/{after_dir}/Gen_" + str(generation) + ".sh", 'w')
...
for individual , filename in enumerate(file_list):
    ...
    with open(f"../BENCH_DIR/{root_dir}/{after_dir}/Individuals/" + filename, 'r') as file:
        for line in file:
    ...
    with open(f"../BENCH_DIR/{root_dir}/{after_dir}/benchmarks.txt", 'w') as bfile:
...
subprocess.call("chmod u+x " + f"../BENCH_DIR/{root_dir}/{after_dir}/Gen_" + str(generation) + ".sh", shell=True)
subprocess.call(f"../BENCH_DIR/{root_dir}/{after_dir}/Gen_" + str(generation) + ".sh", shell=True)
