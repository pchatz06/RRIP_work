...
// RRIP_POLICIES
extern uint64_t rrip_policies;
extern uint64_t dirty_rrip_policies;
extern uint32_t psel_width;
extern uint32_t track_set;
extern uint32_t psel_mask;
extern int dsalgo;
extern int window;
extern int sr_thr;
extern int br_thr;
extern int boost;
extern int hit_mask;
extern uint64_t demote_mask;
extern uint32_t max_count_rr;
...
extern uint64_t global_instr_unique_id;
...
class CACHE : public MEMORY {
  public:
    ...
    uint8_t cache_type;
    uint32_t index_cpu, rr_cpu, rr_counter; //Variables for round robin arbitration
	...
	rr_cpu = 0;
	rr_counter = 0;
        char *name = new char[NAME.length()+1];
	...
	if(strcmp(name,"LLC") == 0){
		for(int i=0;i<NUM_CPUS;i++){
			RQ[i].NAME = NAME + "_RQ" + to_string(i);
			RQ[i].SIZE = RQ_SIZE;
			RQ[i].entry = new PACKET[RQ_SIZE];
		}
	}else{
		RQ[0].NAME = NAME + "_RQ";
        	RQ[0].SIZE = RQ_SIZE;
	        RQ[0].entry = new PACKET[RQ_SIZE];
    ...
    void add_mshr(PACKET *packet),
         ...
         llc_update_replacement_state_back(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint64_t ip, uint64_t victim_addr, uint32_t type, uint8_t hit, uint64_t current_core_cycle, int fill_level),
         llc_update_replacement_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint64_t ip, uint64_t victim_addr, uint32_t type, uint8_t hit, uint64_t current_core_cycle, int fill_level, uint8_t valid),
    ...
    uint32_t get_set(uint64_t address),
             ...
             llc_find_victim_back(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type, uint64_t current_core_cycle, uint64_t instr_unique_id),
             llc_find_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type, uint64_t current_core_cycle, uint64_t instr_unique_id),
