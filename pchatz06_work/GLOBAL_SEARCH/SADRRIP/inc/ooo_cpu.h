...
class O3_CPU {
  public:
    ...
    uint64_t loads_live_cycles[1502]; //GIANNIS cycles a load instructions was live array
    uint64_t loads_retired;
    uint64_t instr_retired;
	
    //Number of Memory Instructions GIANNIS
    uint64_t num_loads,num_stores,total_dest_operands,total_source_operands;	
    uint64_t num_loads_ROI,num_stores_ROI,total_dest_operands_ROI,total_source_operands_ROI;

    //Number of MERGED TRANSLATIONS GIANNIS
    uint64_t DEPENDED_LOADS,SQ_ADDED,LQ_ADDED;
    uint64_t DEPENDED_LOADS_ROI,SQ_ADDED_ROI,LQ_ADDED_ROI;
	...
	for(uint32_t i=0;i < 1502;i++) {
		loads_live_cycles[i] = 0;
	}
        RTS0_head = 0;
        RTS1_head = 0;
        RTS0_tail = 0;
