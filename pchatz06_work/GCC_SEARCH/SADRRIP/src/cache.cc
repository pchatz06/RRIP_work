...
uint64_t global_instr_unique_id;
...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if (cache_type == IS_LLC) {
            way = llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type, current_core_cycle[fill_cpu], global_instr_unique_id);
        }
        else
        ...
        if ((cache_type == IS_LLC) && (way == LLC_WAY)) { // this is a bypass that does not fill the LLC
            ...
            if (cache_type == IS_LLC) {
                llc_update_replacement_state(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, 0, MSHR.entry[mshr_index].type, 0, current_core_cycle[fill_cpu], MSHR.entry[mshr_index].fill_level, block[set][way].valid);
        ...
        if (do_fill) {
            ...
            // update replacement policy
            if (cache_type == IS_LLC) {
                llc_update_replacement_state(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, block[set][way].full_addr, MSHR.entry[mshr_index].type, 0, current_core_cycle[fill_cpu], MSHR.entry[mshr_index].fill_level, block[set][way].valid);
...
void CACHE::handle_writeback()
{
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        if (way >= 0) { // writeback hit (or RFO hit for L1D)
            ...
            if (cache_type == IS_LLC) {
                llc_update_replacement_state(writeback_cpu, set, way, block[set][way].full_addr, WQ.entry[index].ip, 0, WQ.entry[index].type, 1, current_core_cycle[writeback_cpu], WQ.entry[index].fill_level, block[set][way].valid);
            ...
            if (cache_type == IS_L1D) { // RFO miss
                ...
                if (cache_type == IS_LLC) {
                    way = llc_find_victim(writeback_cpu, WQ.entry[index].instr_id, set, block[set], WQ.entry[index].ip, WQ.entry[index].full_addr, WQ.entry[index].type, current_core_cycle[writeback_cpu], global_instr_unique_id);
                }
                else
                ...
                if (do_fill) {
                    ...
                    // update replacement policy
                    if (cache_type == IS_LLC) {
                        llc_update_replacement_state(writeback_cpu, set, way, WQ.entry[index].full_addr, WQ.entry[index].ip, block[set][way].full_addr, WQ.entry[index].type, 0, current_core_cycle[writeback_cpu], WQ.entry[index].fill_level, block[set][way].valid);
...
{
    uint32_t read_cpu = 4;
    uint32_t found_request = 0;


    if(cache_type == 6){
        if(rr_counter < max_count_rr){
		rr_counter++;
        }else{
                rr_cpu = (rr_cpu + 1) % NUM_CPUS;
		rr_counter = 1;
        }
	index_cpu = rr_cpu;
    }
    ...
    /*if(cache_type == 6){
        cout << "RR_STATE" << endl;
        cout << "C0: " << RQ[0].occupancy << endl;
        cout << "C1: " << RQ[1].occupancy << endl;
        cout << "C2: " << RQ[2].occupancy << endl;
        cout << "C3: " << RQ[3].occupancy << endl;
        cout << "Pointer: " << rr_cpu << endl;
    }*/
    ...
    if(cache_type != 6){
	index_cpu = 0;
    	read_cpu = RQ[index_cpu].entry[RQ[index_cpu].head].cpu;
	...
	found_request = 0;
        // handle the oldest entry
        if(cache_type != 6){
		if ((RQ[index_cpu].entry[RQ[index_cpu].head].event_cycle <= current_core_cycle[read_cpu]) && (RQ[index_cpu].occupancy > 0)){
			found_request = 1;
		}
		...
		for(int k=0;k<NUM_CPUS;k++){
			read_cpu = RQ[index_cpu].entry[RQ[index_cpu].head].cpu;
			if(read_cpu == NUM_CPUS){
				index_cpu = (index_cpu + 1) % NUM_CPUS;
				continue;
			}
			if ((RQ[index_cpu].entry[RQ[index_cpu].head].event_cycle <= current_core_cycle[read_cpu]) && (RQ[index_cpu].occupancy > 0)){
				found_request = 1;
        ...
        if ((found_request == 1)) {
            int index = RQ[index_cpu].head;
	    //if(cache_type == IS_LLC)
		//cout << "handle_read_cpu: " << RQ.entry[RQ.head].cpu << endl;
            // access cache
            uint32_t set = get_set(RQ[index_cpu].entry[index].address);
            int way = check_hit(&RQ[index_cpu].entry[index]);
            
            if (way >= 0) { // read hit

                if (cache_type == IS_ITLB) {
                    RQ[index_cpu].entry[index].instruction_pa = block[set][way].data;
                    if (PROCESSED.occupancy < PROCESSED.SIZE)
                        PROCESSED.add_queue(&RQ[index_cpu].entry[index]);
                }
                else if (cache_type == IS_DTLB) {
                    RQ[index_cpu].entry[index].data_pa = block[set][way].data;
                    if (PROCESSED.occupancy < PROCESSED.SIZE)
                        PROCESSED.add_queue(&RQ[index_cpu].entry[index]);
                }
                else if (cache_type == IS_STLB) 
                    RQ[index_cpu].entry[index].data = block[set][way].data;
                else if (cache_type == IS_L1I) {
                    if (PROCESSED.occupancy < PROCESSED.SIZE)
                        PROCESSED.add_queue(&RQ[index_cpu].entry[index]);
                }
                //else if (cache_type == IS_L1D) {
                else if ((cache_type == IS_L1D) && (RQ[index_cpu].entry[index].type != PREFETCH)) {
                    if (PROCESSED.occupancy < PROCESSED.SIZE)
                        PROCESSED.add_queue(&RQ[index_cpu].entry[index]);
                }

                // update prefetcher on load instruction
                if (RQ[index_cpu].entry[index].type == LOAD) {
                    if (cache_type == IS_L1D) 
                        l1d_prefetcher_operate(block[set][way].full_addr, RQ[index_cpu].entry[index].ip, 1, RQ[index_cpu].entry[index].type);
                    else if (cache_type == IS_L2C)
                        l2c_prefetcher_operate(block[set][way].full_addr, RQ[index_cpu].entry[index].ip, 1, RQ[index_cpu].entry[index].type);
                }

                // update replacement policy
                if (cache_type == IS_LLC) {
                    llc_update_replacement_state(read_cpu, set, way, block[set][way].full_addr, RQ[index_cpu].entry[index].ip, 0, RQ[index_cpu].entry[index].type, 1, current_core_cycle[read_cpu], RQ[index_cpu].entry[index].fill_level, block[set][way].valid);

                }
                else
                    update_replacement_state(read_cpu, set, way, block[set][way].full_addr, RQ[index_cpu].entry[index].ip, 0, RQ[index_cpu].entry[index].type, 1);

                // COLLECT STATS
                sim_hit[read_cpu][RQ[index_cpu].entry[index].type]++;
                sim_access[read_cpu][RQ[index_cpu].entry[index].type]++;

                // check fill level
                if (RQ[index_cpu].entry[index].fill_level < fill_level) {

                    if (RQ[index_cpu].entry[index].instruction) 
                        upper_level_icache[read_cpu]->return_data(&RQ[index_cpu].entry[index]);
                    else // data
                        upper_level_dcache[read_cpu]->return_data(&RQ[index_cpu].entry[index]);
                }
                ...
                HIT[RQ[index_cpu].entry[index].type]++;
                ACCESS[RQ[index_cpu].entry[index].type]++;
                
                // remove this entry from RQ
                RQ[index_cpu].remove_queue(&RQ[index_cpu].entry[index]);
                ...
                cout << " instr_id: " << RQ[index_cpu].entry[index].instr_id << " address: " << hex << RQ[index_cpu].entry[index].address;
                cout << " full_addr: " << RQ[index_cpu].entry[index].full_addr << dec;
                cout << " cycle: " << RQ[index_cpu].entry[index].event_cycle << endl; });

                // check mshr
                uint8_t miss_handled = 1;
                int mshr_index = check_mshr(&RQ[index_cpu].entry[index]);

                if ((mshr_index == -1) && (MSHR.occupancy < MSHR_SIZE)) { // this is a new miss

                    // add it to mshr (read miss)
                    add_mshr(&RQ[index_cpu].entry[index]);

                    // add it to the next level's read queue
                    if (lower_level)
                        lower_level->add_rq(&RQ[index_cpu].entry[index]);
                    else { // this is the last level
                        if (cache_type == IS_STLB) {
                            // TODO: need to differentiate page table walk and actual swap

                            // emulate page table walk
                            uint64_t pa = va_to_pa(read_cpu, RQ[index_cpu].entry[index].instr_id, RQ[index_cpu].entry[index].full_addr, RQ[index_cpu].entry[index].address);
			    //if(all_warmup_complete < NUM_CPUS) 
			    //cout << "PAGE MAP CPU: " << read_cpu << " VA: " <<  hex << RQ.entry[index].address << " TO PA: " << hex << pa << endl;
                            
                            RQ[index_cpu].entry[index].data = pa >> LOG2_PAGE_SIZE; 
                            RQ[index_cpu].entry[index].event_cycle = current_core_cycle[read_cpu];
                            return_data(&RQ[index_cpu].entry[index]);
                        }
                    }
                }
                else {
                    if ((mshr_index == -1) && (MSHR.occupancy == MSHR_SIZE)) { // not enough MSHR resource
                        ...
                        // cannot handle miss request until one of MSHRs is available
                        miss_handled = 0;
                        STALL[RQ[index_cpu].entry[index].type]++;
                    }
                    else if (mshr_index != -1) { // already in-flight miss

                        // mark merged consumer
                        if (RQ[index_cpu].entry[index].type == RFO) {

                            if (RQ[index_cpu].entry[index].tlb_access) {
                                uint32_t sq_index = RQ[index_cpu].entry[index].sq_index;
                                MSHR.entry[mshr_index].store_merged = 1;
                                MSHR.entry[mshr_index].sq_index_depend_on_me.insert (sq_index);
				MSHR.entry[mshr_index].sq_index_depend_on_me.join (RQ[index_cpu].entry[index].sq_index_depend_on_me, SQ_SIZE);
                            }

                            if (RQ[index_cpu].entry[index].load_merged) {
                                //uint32_t lq_index = RQ[index_cpu].entry[index].lq_index; 
                                MSHR.entry[mshr_index].load_merged = 1;
                                //MSHR.entry[mshr_index].lq_index_depend_on_me[lq_index] = 1;
				MSHR.entry[mshr_index].lq_index_depend_on_me.join (RQ[index_cpu].entry[index].lq_index_depend_on_me, LQ_SIZE);
                            }
                        }
                        else {
                            if (RQ[index_cpu].entry[index].instruction) {
                                uint32_t rob_index = RQ[index_cpu].entry[index].rob_index;
                                ...
                                cout << " merged rob_index: " << rob_index << " instr_id: " << RQ[index_cpu].entry[index].instr_id << endl; });

                                if (RQ[index_cpu].entry[index].instr_merged) {
				    MSHR.entry[mshr_index].rob_index_depend_on_me.join (RQ[index_cpu].entry[index].rob_index_depend_on_me, ROB_SIZE);
                                    DP (if (warmup_complete[MSHR.entry[mshr_index].cpu]) {
                                    cout << "[INSTR_MERGED] " << __func__ << " cpu: " << MSHR.entry[mshr_index].cpu << " instr_id: " << MSHR.entry[mshr_index].instr_id;
                            ...
                            else 
                            {
                                ...
                                DP (if (warmup_complete[read_cpu]) {
                                cout << "[DATA_MERGED] " << __func__ << " cpu: " << read_cpu << " instr_id: " << RQ[index_cpu].entry[index].instr_id;
                                cout << " merged rob_index: " << RQ[index_cpu].entry[index].rob_index << " instr_id: " << RQ[index_cpu].entry[index].instr_id << " lq_index: " << RQ[index_cpu].entry[index].lq_index << endl; });
				MSHR.entry[mshr_index].lq_index_depend_on_me.join (RQ[index_cpu].entry[index].lq_index_depend_on_me, LQ_SIZE);
                                if (RQ[index_cpu].entry[index].store_merged) {
                                    MSHR.entry[mshr_index].store_merged = 1;
				    MSHR.entry[mshr_index].sq_index_depend_on_me.join (RQ[index_cpu].entry[index].sq_index_depend_on_me, SQ_SIZE);
                                }
                            }
                        ...
                        // update fill_level
                        if (RQ[index_cpu].entry[index].fill_level < MSHR.entry[mshr_index].fill_level)
                            MSHR.entry[mshr_index].fill_level = RQ[index_cpu].entry[index].fill_level;

                        // update request
                        if (MSHR.entry[mshr_index].type == PREFETCH) {
                            uint8_t  prior_returned = MSHR.entry[mshr_index].returned;
                            uint64_t prior_event_cycle = MSHR.entry[mshr_index].event_cycle;
                            MSHR.entry[mshr_index] = RQ[index_cpu].entry[index];
                        ...
                        MSHR_MERGED[RQ[index_cpu].entry[index].type]++;

                        DP ( if (warmup_complete[read_cpu]) {
                        cout << "[" << NAME << "] " << __func__ << " mshr merged";
                        cout << " instr_id: " << RQ[index_cpu].entry[index].instr_id << " prior_id: " << MSHR.entry[mshr_index].instr_id; 
                        cout << " address: " << hex << RQ[index_cpu].entry[index].address;
                        cout << " full_addr: " << RQ[index_cpu].entry[index].full_addr << dec;
                        cout << " cycle: " << RQ[index_cpu].entry[index].event_cycle << endl; });
                    }
                    else { // WE SHOULD NOT REACH HERE
                ...
                if (miss_handled) {
                    ...
                    if (RQ[index_cpu].entry[index].type == LOAD) {
                        if (cache_type == IS_L1D) 
                            l1d_prefetcher_operate(RQ[index_cpu].entry[index].full_addr, RQ[index_cpu].entry[index].ip, 0, RQ[index_cpu].entry[index].type);
                        if (cache_type == IS_L2C)
                            l2c_prefetcher_operate(RQ[index_cpu].entry[index].full_addr, RQ[index_cpu].entry[index].ip, 0, RQ[index_cpu].entry[index].type);
                    }

                    MISS[RQ[index_cpu].entry[index].type]++;
                    ACCESS[RQ[index_cpu].entry[index].type]++;
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
        ...
        if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0)) {
            ...
            if (way >= 0) { // prefetch hit
                ...
                // update replacement policy
                if (cache_type == IS_LLC) {
                    llc_update_replacement_state(prefetch_cpu, set, way, block[set][way].full_addr, PQ.entry[index].ip, 0, PQ.entry[index].type, 1, current_core_cycle[prefetch_cpu], PQ.entry[index].fill_level, block[set][way].valid);
...
void CACHE::operate()
{
    ...
    if(cache_type != 6){
	if (PQ.occupancy && (RQ[0].occupancy == 0)){
		handle_prefetch();
	}
    }else if(cache_type == 6){
	uint32_t llc_occupancy = 0;
	for(int i=0;i < NUM_CPUS;i++){
		llc_occupancy += RQ[i].occupancy;
	}
	if(PQ.occupancy && (llc_occupancy == 0)){
		handle_prefetch();
	}
    }
}
...
int CACHE::add_rq(PACKET *packet)
{
    ...
    if (wq_index != -1) {
        ...
        WQ.FORWARD++;
        RQ[insert_cpu].ACCESS++;

        return -1;
    }

    // check for duplicates in the read queue
    int index = RQ[insert_cpu].check_queue(packet);
    if (index != -1) {
        
        if (packet->instruction) {
            uint32_t rob_index = packet->rob_index;
            RQ[insert_cpu].entry[index].rob_index_depend_on_me.insert (rob_index);
            RQ[insert_cpu].entry[index].instr_merged = 1;

            DP (if (warmup_complete[packet->cpu]) {
            cout << "[INSTR_MERGED] " << __func__ << " cpu: " << packet->cpu << " instr_id: " << RQ[insert_cpu].entry[index].instr_id;
        ...
        else 
        {
            ...
            if (packet->type == RFO) {
                ...
                RQ[insert_cpu].entry[index].sq_index_depend_on_me.insert (sq_index);
                RQ[insert_cpu].entry[index].store_merged = 1;
            }
            else {
                uint32_t lq_index = packet->lq_index; 
                RQ[insert_cpu].entry[index].lq_index_depend_on_me.insert (lq_index);
                RQ[insert_cpu].entry[index].load_merged = 1;

                DP (if (warmup_complete[packet->cpu]) {
                cout << "[DATA_MERGED] " << __func__ << " cpu: " << packet->cpu << " instr_id: " << RQ[insert_cpu].entry[index].instr_id;
                cout << " merged rob_index: " << packet->rob_index << " instr_id: " << packet->instr_id << " lq_index: " << packet->lq_index << endl; });
            }
        }

        RQ[insert_cpu].MERGED++;
        RQ[insert_cpu].ACCESS++;
    ...
    // check occupancy
    if (RQ[insert_cpu].occupancy == RQ_SIZE) {
        RQ[insert_cpu].FULL++;
    ...
    index = RQ[insert_cpu].tail;

#ifdef SANITY_CHECK
    if (RQ[insert_cpu].entry[index].address != 0) {
        cerr << "[" << NAME << "_ERROR] " << __func__ << " is not empty index: " << index;
        cerr << " address: " << hex << RQ[insert_cpu].entry[index].address;
        cerr << " full_addr: " << RQ[insert_cpu].entry[index].full_addr << dec << endl;
        assert(0);
    }
#endif

    RQ[insert_cpu].entry[index] = *packet;

    // ADD LATENCY
    if (RQ[insert_cpu].entry[index].event_cycle < current_core_cycle[packet->cpu])
        RQ[insert_cpu].entry[index].event_cycle = current_core_cycle[packet->cpu] + LATENCY;
    else
        RQ[insert_cpu].entry[index].event_cycle += LATENCY;

    RQ[insert_cpu].occupancy++;
    RQ[insert_cpu].tail++;
    if (RQ[insert_cpu].tail >= RQ[insert_cpu].SIZE)
        RQ[insert_cpu].tail = 0;

    DP ( if (warmup_complete[RQ[insert_cpu].entry[index].cpu]) {
    cout << "[" << NAME << "_RQ] " <<  __func__ << " instr_id: " << RQ[insert_cpu].entry[index].instr_id << " address: " << hex << RQ[insert_cpu].entry[index].address;
    cout << " full_addr: " << RQ[insert_cpu].entry[index].full_addr << dec;
    cout << " type: " << +RQ[insert_cpu].entry[index].type << " head: " << RQ[insert_cpu].head << " tail: " << RQ[insert_cpu].tail << " occupancy: " << RQ[insert_cpu].occupancy;
    cout << " event: " << RQ[insert_cpu].entry[index].event_cycle << " current: " << current_core_cycle[RQ[insert_cpu].entry[insert].cpu] << endl; });

    if (packet->address == 0)
        assert(0);

    RQ[insert_cpu].TO_CACHE++;
    RQ[insert_cpu].ACCESS++;
